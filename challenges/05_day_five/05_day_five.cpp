...
public:
    Cloud(int cloud_id, string params) {
    ...
    void print_coords() {
        cout << "Start: " << "X: " << this->x_start << " Y:" << this->y_start << endl;
        cout << "End: " << "X: " << this->x_end << " Y:" << this->y_end << endl;
    }
...
class DayFive : public BaseChallenge {
private:
    ...
    void part_one() override {
        ...
        for (const auto &line: input_buffer) {
            Cloud cloud = Cloud(cloud_id, line);
...
public:
    DayFive() : BaseChallenge("05_day_five.txt", "Day 5: Day Five") {}
};
