...
#include "04_giant_squid.h"

void GiantSquid::part_one() {
    ...
    vector<vector<string>> boards = GiantSquid::get_boards();
...
vector<vector<string>> GiantSquid::get_boards() {
...
void GiantSquid::print_board(vector<string> board, const string &drawn_number) {
    cout << "-----" << endl;
...
bool GiantSquid::is_winner(vector<string> board) {
    // Check rows
    for (int row_index = 0; row_index < 5; row_index++) {
        int counter = 0;
        for (int position = 0; position < 5; position++) {
...
GiantSquid::GiantSquid() : BaseChallenge("04_giant_squid.txt", "Day 4: Giant Squid") {}